/**
 * Campagne d'optimisation / validation moteur Driveely.
 *
 ...
 *   NEXT_PUBLIC_APP_URL=http://localhost:3000 npx tsx scripts/campaign-driveely-optimize.ts
 *
 * Options env :
 *   DRIVEELY_CAMPAIGN_ROUNDS=3     # répétitions Vision par image (défaut 2)
 *   DRIVEELY_CAMPAIGN_PROFILES=1   # 1 = matrice profils locale (sans re-Vision)
 */
...
const ROUNDS = Math.max(1, Number(process.env.DRIVEELY_CAMPAIGN_ROUNDS ?? 2));
const RUN_PROFILES = process.env.DRIVEELY_CAMPAIGN_PROFILES !== "0";
...
async function setupUser() {
  const email = `driveely.campaign.${Date.now()}@gmail.com`;
...
async function main() {
  console.log(`\n=== Campagne optimisation Driveely — ${BASE} ===\n`);
  ...
  const outPath = path.join(__dirname, "campaign-driveely-optimize-report.json");
