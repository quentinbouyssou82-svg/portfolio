/**
 * QA Uberly — parcours bêta-testeur réel (prod ou local).
 * Usage :
 *   set -a && source .env.local && set +a
 *   NEXT_PUBLIC_APP_URL=https://margeo.vercel.app npx tsx scripts/qa-uberly-beta.ts
 */
...
(async () => {
  ...
  const email = `uberly.qa.${Date.now()}@gmail.com`;
  ...
  try {
    // --- Health ---
    const healthRes = await fetch(`${BASE}/api/uberly/health`);
    ...
    const unauth = await fetch(`${BASE}/api/uberly/analyze`, { method: "POST" });
    ...
    const dashAnon = await fetch(`${BASE}/demos/uberly/dashboard`, {
      redirect: "manual",
    });
    ...
    await page.goto(`${BASE}/demos/uberly/login?mode=signup`, {
    ...
    await page2.goto(`${BASE}/demos/uberly/dashboard`, { waitUntil: "networkidle" });
    ...
    // Invalid: no image
    const noImg = await fetch(`${BASE}/api/uberly/analyze`, {
      method: "POST",
    ...
    const badRes = await fetch(`${BASE}/api/uberly/analyze`, {
      method: "POST",
      headers: { Cookie: cookie },
      body: bad,
    });
    ...
    const bigRes = await fetch(`${BASE}/api/uberly/analyze`, {
      method: "POST",
      headers: { Cookie: cookie },
      body: bigForm,
    });
    ...
    // Historique UI
    await page2.goto(`${BASE}/demos/uberly/historique`, { waitUntil: "networkidle" });
    ...
    await page2.goto(`${BASE}/demos/uberly/analyse`, { waitUntil: "networkidle" });
