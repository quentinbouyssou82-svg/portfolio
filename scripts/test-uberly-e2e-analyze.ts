/**
 * E2E backend Uberly : user admin → onboarding DB → analyse Mistral.
 * Usage : npx tsx scripts/test-uberly-e2e-analyze.ts
 */
...
const email = `uberly-e2e-${Date.now()}@test.local`;
...
function buildTestScreenshotPng(): Buffer {
  ...
  const testPath = path.join(__dirname, "fixtures/uberly-test-screenshot.png");
...
(async () => {
  console.log("E2E Uberly analyze —", email);
