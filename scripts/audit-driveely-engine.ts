/**
 * Audit moteur Driveely — captures Vision + robustesse profils + cohérence score.
 *
 * Usage :
 *   set -a && source .env.local && set +a
 *   npx tsx scripts/audit-driveely-engine.ts
 *   NEXT_PUBLIC_APP_URL=http://localhost:3000 npx tsx scripts/audit-driveely-engine.ts
 */
...
async function setupUser() {
  const email = `driveely.audit.${Date.now()}@gmail.com`;
...
async function testGeolocation(access: string, refresh: string, userId: string) {
  ...
  const getEmpty = await fetch(`${BASE}/api/driveely/location`, {
  ...
  const postGrant = await fetch(`${BASE}/api/driveely/location`, {
    method: "POST",
  ...
  const getAfter = await fetch(`${BASE}/api/driveely/location`, {
    headers: { Cookie: cookie },
  });
  ...
  const postDeny = await fetch(`${BASE}/api/driveely/location`, {
    method: "POST",
    headers: { Cookie: cookie, "Content-Type": "application/json" },
...
async function testVisionCaptures(access: string, refresh: string) {
  ...
  const reportPath = path.join(__dirname, "audit-driveely-engine-report.json");
  fs.writeFileSync(reportPath, JSON.stringify(results, null, 2));
  pass("rapport-json", reportPath);
}

async function main() {
  console.log(`\n=== Audit moteur Driveely — ${BASE} ===\n`);
