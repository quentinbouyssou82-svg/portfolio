/**
 * Test bêta Driveely complet : auth + analyse + historique.
 * Usage : set -a && source .env.local && set +a && npx tsx scripts/test-driveely-beta-e2e.ts
 */
...
const email = `driveely.beta.${Date.now()}@gmail.com`;
...
(async () => {
  try {
    ...
    const analyzeRes = await fetch(`${BASE}/api/driveely/analyze`, {
      method: "POST",
    ...
    const histRes = await fetch(`${BASE}/demos/driveely/historique`, {
      headers: { Cookie: cookieHeader(access, refresh) },
      redirect: "manual",
    });
    ...
    if (analysisId) {
      const detailRes = await fetch(`${BASE}/demos/driveely/historique/${analysisId}`, {
        headers: { Cookie: cookieHeader(access, refresh) },
        redirect: "manual",
      });
    ...
    const dash = await fetch(`${BASE}/demos/driveely/dashboard`, {
      headers: { Cookie: cookieHeader(access, refresh) },
      redirect: "manual",
    });
