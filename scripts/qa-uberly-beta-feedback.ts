 ...
 *   NEXT_PUBLIC_APP_URL=http://127.0.0.1:3000 npx tsx scripts/qa-uberly-beta-feedback.ts
 */
...
async function loginUi(page: Page, email: string, password: string) {
  await page.goto(`${BASE}/demos/uberly/login`, {
...
async function main() {
  ...
  const email = `uberly.feedback.${Date.now()}@gmail.com`;
  ...
  try {
    ...
    // 1. Landing dates (desktop)
    {
      ...
      await page.goto(`${BASE}/demos/uberly`, { waitUntil: "networkidle" });
    ...
    for (const [name, ctx] of [
      ...
      await page.goto(`${BASE}/demos/uberly/analyse`, {
        waitUntil: "domcontentloaded",
        timeout: 60000,
      });
      ...
      // Profil
      await page.goto(`${BASE}/demos/uberly/profil`, {
        waitUntil: "domcontentloaded",
        timeout: 60000,
      });
