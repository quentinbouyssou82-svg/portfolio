/**
 * Parcours utilisateur réel Uberly (navigateur Playwright).
 * Usage : set -a && source .env.local && set +a && npx tsx scripts/test-uberly-user-journey.ts
 */
...
const email = `uberly.browser.${Date.now()}@gmail.com`;
...
async function waitForDashboard(page: Page, timeout = 30_000) {
  await page.waitForURL(/\/demos\/uberly\/dashboard/, { timeout });
  await page.getByText("Centre de pilotage").waitFor({ timeout });
}

async function completeOnboarding(page: Page) {
  await page.getByText("Bienvenue sur Uberly").waitFor({ timeout: 15_000 });
...
(async () => {
  ...
  const storagePath = path.join(__dirname, ".uberly-session.json");
  ...
  try {
    ...
    if (
      !(await runStep(1, async () => {
        await page!.goto(`${BASE}/demos/uberly/login?mode=signup`, { waitUntil: "networkidle" });
    ...
    if (
      !(await runStep(2, async () => {
        await page!.waitForURL(/\/demos\/uberly\/(onboarding|dashboard)/, { timeout: 45_000 });
    ...
    if (
      !(await runStep(3, async () => {
        await page!.waitForURL(/\/demos\/uberly\/onboarding/, { timeout: 15_000 });
        await page!.getByText("Bienvenue sur Uberly").waitFor();
      }))
    ) {
      return;
    }
    ...
    if (
      !(await runStep(7, async () => {
        ...
        page = await context.newPage();
        await page.goto(`${BASE}/demos/uberly/dashboard`, { waitUntil: "networkidle" });
        await waitForDashboard(page);
      }))
    ) {
      return;
    ...
    if (
      !(await runStep(9, async () => {
        await page!.goto(`${BASE}/demos/uberly/deconnexion`, { waitUntil: "networkidle" });
        await page!.waitForURL(/\/demos\/uberly\/login/, { timeout: 15_000 });
      }))
    ) {
      return;
    }
    ...
    if (
      !(await runStep(10, async () => {
        ...
        await page!.waitForURL(/\/demos\/uberly\/(dashboard|onboarding)/, { timeout: 45_000 });
      }))
    ) {
      return;
    }
