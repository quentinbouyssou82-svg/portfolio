/**
 * Test fonctionnel API Driveely (signup admin → onboarding → analyse → feedback).
 * Usage : set -a && source .env.local && set +a && npx tsx scripts/test-driveely-full-flow.ts
 */
...
async function feedback(access: string, refresh: string, analysisId: string) {
  const res = await fetch(`${BASE}/api/driveely/feedback`, {
    method: "POST",
    headers: {
