/**
 * QA abonnement Driveely (activation simulée).
 * Usage: set -a && source .env.local && set +a && npx tsx scripts/qa-driveely-subscription.ts
 */
...
const APP =
  ...
  "http://localhost:3000";
const BASE = `${APP}/demos/driveely`;
const API = `${APP}/api/driveely`;
...
async function main() {
  console.log(`\n=== QA Abonnement Driveely — ${APP} ===\n`);

  const admin = createClient(url, service, {
    auth: { persistSession: false, autoRefreshToken: false },
  });

  const email = `sub-qa-${Date.now()}@driveely.test`;
