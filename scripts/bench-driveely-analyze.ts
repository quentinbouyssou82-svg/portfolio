/**
 * Benchmark pipeline analyse Driveely (timings étape par étape).
 * Usage : set -a && source .env.local && set +a && npx tsx scripts/bench-driveely-analyze.ts
 */
...
async function setupUser() {
  const email = `driveely.bench.${Date.now()}@gmail.com`;
