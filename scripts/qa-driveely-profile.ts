/**
 * Smoke test profil + logout Driveely (bêta).
 * Usage: npx tsx scripts/qa-driveely-profile.ts
 */
...
const email = `profile-qa-${Date.now()}@driveely-test.local`;
...
async function uploadAvatar(userId: string) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/storage/v1/object/driveely-avatars/${path}`,
    {
      method: "POST",
      headers: {
        apikey: SECRET,
  ...
  const publicUrl = `${SUPABASE_URL}/storage/v1/object/public/driveely-avatars/${path}`;
  return publicUrl;
}
...
async function main() {
  console.log("QA Driveely profile @", APP);
  ...
  if (!meta.avatar_url?.includes("driveely-avatars")) {
  ...
  console.log(
    "NOTE: run supabase/driveely-profile-v1.sql to add first_name/last_name/avatar_url columns on margeo_profiles.",
  );
}
