#!/usr/bin/env node
/**
 * Smoke test endpoints Uberly (sans auth).
 * Usage : node scripts/uberly-smoke.mjs [baseUrl]
...
console.log(`Uberly smoke — ${base}\n`);

let ok = true;
ok = (await check("/api/uberly/health", "GET /api/uberly/health")) && ok;

const health = await fetch(`${base}/api/uberly/health`).then((r) => r.json()).catch(() => null);
if (health) {
