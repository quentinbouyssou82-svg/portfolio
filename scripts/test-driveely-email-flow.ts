/**
 * Test parcours auth email Driveely (signup → onboarding → dashboard → logout → login).
 * Usage : set -a && source .env.local && set +a && npx tsx scripts/test-driveely-email-flow.ts
 */
...
const email = `driveely.flow.${Date.now()}@gmail.com`;
...
(async () => {
  ...
  try {
    ...
    const onboardingRes = await fetch(`${BASE}/demos/driveely/onboarding`, {
    ...
    const dashRes = await fetch(`${BASE}/demos/driveely/dashboard`, {
      headers: { Cookie: cookieHeader(access, refresh) },
      redirect: "manual",
    });
    ...
    const dash2 = await fetch(`${BASE}/demos/driveely/dashboard`, {
      headers: { Cookie: cookieHeader(access, refresh) },
      redirect: "manual",
    });
