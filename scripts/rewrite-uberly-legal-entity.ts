/**
 * Corrige le statut juridique dans les documents Uberly :
...
const DIR = path.join(__dirname, "../content/uberly/legal");

const ENTITY_SHORT =
  "Uberly, micro-entreprise [SIREN — EN COURS D'IMMATRICULATION]";
const ENTITY_LONG =
...
*Précision relative à l'organisation juridique : l'activité Uberly est exploitée par une micro-entreprise française créée et dirigée par le tuteur légal de M. Leandro Machado (son père), qui en est le responsable légal. M. Leandro Machado participe au développement et au projet en qualité de porteur du projet, sans être le représentant légal de la micro-entreprise.*`,
    )
    .replace(
      /Le directeur de la publication est M\. Leandro Machado, Président de la SASU Uberly, agissant conformément à l'autorisation visée à l'article 1 ci-dessus\./,
      "Le directeur de la publication est [NOM DU TUTEUR LÉGAL — À COMPLÉTER], responsable légal de la micro-entreprise exploitant le Service Uberly. M. Leandro Machado contribue au projet en qualité de porteur du projet, sans être le représentant légal de l'entreprise.",
    );
}
...
function rewriteGeneric(content: string, file: string): string {
  ...
  out = out.replace(
    /édité par la SASU Uberly \[SIREN — EN COURS D'IMMATRICULATION\]/g,
    "édité par Uberly, micro-entreprise [SIREN — EN COURS D'IMMATRICULATION]",
  );
  out = out.replace(
    /édité par Uberly, SASU \[SIREN — EN COURS D'IMMATRICULATION\]/g,
    "édité par Uberly, micro-entreprise [SIREN — EN COURS D'IMMATRICULATION]",
  );
  out = out.replace(
    /Uberly, SASU \[SIREN — EN COURS D'IMMATRICULATION\]/g,
    ENTITY_SHORT,
  );
  out = out.replace(
    /Uberly, SASU en cours d'immatriculation/g,
    "Uberly, micro-entreprise en cours d'immatriculation",
  );
  out = out.replace(
    /applicables à Uberly, SASU/g,
    "applicables à Uberly, micro-entreprise",
  );
  out = out.replace(
    /édité par Uberly, SASU \[SIREN — EN COURS D'IMMATRICULATION\]/g,
  ...
  if (file === "00-README.md") {
    out = `# Documents juridiques Uberly — Récapitulatif

Ce dossier contient les 13 documents juridiques nécessaires au lancement de la bêta privée puis de la commercialisation d'Uberly en France, cohérents entre eux et fondés sur la structure retenue : **micro-entreprise française** exploitant l'enseigne Uberly, **dirigée par le tuteur légal** de Leandro Machado (son père). Leandro Machado est le **porteur du projet** (développement / opérationnel) mais **n'est pas le représentant légal** de la micro-entreprise.
...
const bad = ["Président de la SASU", "SASU Uberly", "dirigeant mineur", "acte d'autorisation parentale d'exercer"];
for (const file of fs.readdirSync(DIR).filter((f) => f.endsWith(".md"))) {
