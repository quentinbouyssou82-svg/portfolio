/**
 * Test auth Uberly (signup → session → logout) via Supabase + API simulée.
 * Usage : set -a && source .env.local && set +a && npx tsx scripts/test-uberly-auth.ts
 */
...
const email = `uberly.auth.${Date.now()}@gmail.com`;
...
(async () => {
  try {
    ...
    const quotaAfter = await fetch(`${BASE}/api/uberly/quota`, {
