-- Uberly bootstrap — ne pas modifier à la main
...
-- ═══ uberly-migrate.sql ═══
-- Uberly — migration depuis Margeo
-- Exécuter après margeo-setup.sql
...
-- ═══ uberly-beta.sql ═══
-- Uberly — préparation beta (10-20 livreurs)
-- Exécuter après : margeo-setup.sql → margeo-rls.sql → uberly-migrate.sql → uberly-backend-v2.sql
...
create or replace view public.uberly_beta_stats as
select
...
-- ═══ uberly-beta-v2.sql ═══
-- Uberly beta v2 — événements funnel + testeurs beta
-- Exécuter après uberly-beta.sql
...
create or replace view public.uberly_beta_funnel as
with users as (
...
create or replace view public.uberly_beta_errors as
select
  id,
  user_id,
  event_type,
