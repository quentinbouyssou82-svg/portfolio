# Uberly — Procédure test par livreur beta

## Avant le test (toi)

1. Migrations 1→7 exécutées (voir `UBERLY-MIGRATIONS.md` + `uberly-beta-v3.sql`)
2. `.env` prod configuré, `UBERLY_BETA_MODE=true`
3. `curl https://TON-DOMAINE/api/uberly/health` → `readyForBeta: true`

## Par testeur (15 min)

### Étape 1 — Création compte
- S'inscrire email ou Google sur `/demos/uberly/signup`
...
| 500 save | RLS ou colonnes manquantes | Exécuter `uberly-verify-beta.sql` |
| Pas d'events beta | SERVICE_ROLE manquante | Ajouter clé + redéployer |
| Image non stockée | Bucket absent | Exécuter `uberly-backend-v2.sql` |

## SQL monitoring après session

```sql
select * from uberly_beta_funnel where user_id = 'UUID';
select * from uberly_beta_vision_stats limit 7;
select * from uberly_beta_errors limit 20;
```
