...
export function PhoneMock() {
  return (
    <div className="relative mx-auto w-[300px] rounded-[2.4rem] border border-mg-border-strong bg-mg-surface p-2.5 shadow-[0_24px_80px_rgba(0,0,0,0.55)]">
      <div className="rounded-[1.9rem] border border-mg-border bg-mg-background px-4 pt-5 pb-6">
        ...
        {/* Explication IA */}
        <div className="mt-3 rounded-xl border border-mg-accent/20 bg-mg-accent-soft p-3">
          <p className="text-[11px] leading-relaxed text-mg-foreground/90">
            <span className="font-semibold text-mg-accent">Uberly · </span>
      ...
      {/* Cartes flottantes */}
      <motion.div
        className="absolute -left-24 top-24 hidden rounded-xl border border-mg-border bg-mg-card px-3.5 py-2.5 shadow-mg-card lg:block"
        animate={{ y: [0, -8, 0] }}
        transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
      >
        <p className="text-[10px] text-mg-faint">Gain du jour</p>
        <p className="text-sm font-bold text-mg-accent">+68,40 €</p>
      </motion.div>
      <motion.div
        className="absolute -right-24 bottom-28 hidden rounded-xl border border-mg-border bg-mg-card px-3.5 py-2.5 shadow-mg-card lg:block"
        animate={{ y: [0, 8, 0] }}
        transition={{ duration: 6, repeat: Infinity, ease: "easeInOut", delay: 1 }}
      >
        <p className="text-[10px] text-mg-faint">Courses évitées</p>
        <p className="text-sm font-bold text-mg-foreground">
          9 <span className="text-[10px] font-normal text-mg-stop">non rentables</span>
        </p>
      </motion.div>
    </div>
  );
