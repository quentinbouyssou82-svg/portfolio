...
function Metric({
  ...
  return (
    <Card
      interactive
      className={
        highlight ? "border-mg-accent/30 bg-mg-accent-soft/40 p-4" : "p-4"
      }
    >
...
/**
 * Hiérarchie : Décision → Score → Explication → Détails financiers
 */
      ...
      <motion.div variants={item}>
        ...
        >
          <div
            ...
            style={{
              background: `radial-gradient(520px circle at 50% 0%, color-mix(in srgb, ${verdictMeta.color} 14%, transparent), transparent 70%)`,
            }}
            aria-hidden
          />
          ...
          {analysis.insights.length > 0 && (
            <ul className="mt-4 space-y-2 border-t border-mg-border pt-4">
              {analysis.insights.map((insight) => (
                <li
                  key={insight}
                  className="flex items-start gap-2.5 text-sm text-mg-muted"
                >
                  <span className="mt-1.5 size-1.5 shrink-0 rounded-full bg-mg-accent/60" />
                  {insight}
                </li>
              ))}
            </ul>
