...
export function AppShell({
  ...
  return (
    <div className="min-h-dvh lg:flex">
      ...
      {/* Barre mobile haute */}
      <header className="fixed inset-x-0 top-0 z-40 flex h-14 items-center justify-between border-b border-mg-border bg-mg-background/80 px-4 backdrop-blur-xl lg:hidden">
        <Link href={margeoRoutes.home} aria-label="Retour à l'accueil">
          <Logo />
        </Link>
        <Link
          href={margeoRoutes.profil}
          className="flex size-8 items-center justify-center rounded-full bg-mg-accent-soft text-sm font-semibold text-mg-accent"
        >
          {profile.name.charAt(0)}
        </Link>
      </header>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex min-h-[52px] min-w-[56px] flex-col items-center justify-center gap-0.5 rounded-xl px-2 py-1.5 text-[10px] font-medium transition-colors",
                  active
                    ? "text-mg-accent"
