...
export const SCREENSHOT_BUCKET =
  process.env.DRIVEELY_SCREENSHOT_BUCKET?.trim() ||
  process.env.UBERLY_SCREENSHOT_BUCKET?.trim() ||
  "uberly-screenshots";
const AVATAR_BUCKET =
  process.env.DRIVEELY_AVATAR_BUCKET?.trim() ||
  process.env.UBERLY_AVATAR_BUCKET?.trim() ||
  "uberly-avatars";
...
export async function uploadScreenshot(
  ...
  try {
    ...
    console.warn("[driveely/storage] upload skipped:", e);
    return null;
  }
}
...
export async function uploadScreenshotBuffer(
  ...
  try {
    ...
    if (error) {
      console.warn("[driveely/storage] upload failed:", error.message);
      return null;
    }
...
export async function deleteScreenshot(path: string): Promise<boolean> {
  ...
  try {
    ...
    const { error } = await admin.storage
      ...
      .remove([path]);
    if (error) {
      console.warn("[driveely/storage] delete failed:", error.message);
      return false;
    }
    return true;
  } catch (e) {
    console.warn("[driveely/storage] delete skipped:", e);
    return false;
  }
}
  ...
  try {
    ...
    for (let i = 0; i < unique.length; i += chunkSize) {
      ...
      const { error } = await admin.storage
        ...
        .remove(chunk);
      if (error) {
        console.warn("[driveely/storage] batch delete failed:", error.message);
        continue;
      }
      deleted += chunk.length;
    }
  } catch (e) {
    console.warn("[driveely/storage] batch delete skipped:", e);
  }
  return deleted;
}
...
/**
 * Upload avatar profil (bucket public driveely-avatars).
...
export async function uploadAvatar(
  ...
  try {
    ...
    if (error) {
      console.warn("[driveely/storage] avatar upload failed:", error.message);
      return null;
    }
    ...
    console.warn("[driveely/storage] avatar upload skipped:", e);
    return null;
  }
}
