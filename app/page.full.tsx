/**
 * BACKUP — page d'accueil complète (animations, sections, hooks).
 * Pour restaurer : copier ce fichier vers app/page.tsx
 * ou renommer : mv app/page.full.tsx app/page.tsx
 */
"use client";
...
export default function Home() {
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-[var(--background)] text-[var(--foreground)]">
      ...
      <ParallaxLayer
        speed={0.55}
        className="pointer-events-none absolute -top-32 left-1/2 h-[16rem] w-[16rem] -translate-x-1/2 md:max-lg:h-[22rem] md:max-lg:w-[22rem] lg:h-[35rem] lg:w-[35rem]"
      >
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-16 px-4 pb-8 pt-[4.75rem] max-md:gap-14 max-md:px-4 max-md:pb-8 max-md:pt-[4.5rem] md:max-lg:gap-20 md:px-10 md:pb-20 md:pt-28 lg:gap-28">
        ...
        <section id="services" className="space-y-7 max-md:space-y-6 md:max-lg:space-y-8 lg:space-y-9">
        ...
        <section id="about" className="space-y-5 max-md:space-y-4">
          <CinematicReveal>
            <h2 className="text-[1.75rem] font-semibold leading-tight tracking-[-0.02em] max-md:text-[1.65rem] md:text-5xl">
              L&apos;agence
            </h2>
